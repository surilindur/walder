...
module.exports = class GraphQLLDHandler extends Handler {
  ...
  async handle(queryInfo, pathParams = {}, queryParams = {}) {
    ...
    const queryEngine = await this.getEngineFromCache(queryInfo, parseDataSources);

    const queryContext = {
      ...queryInfo.context,
      ...queryInfo.comunicaConfig,
      queryFormat: {
        language: 'graphql',
        version: '1.0'
      },
    ...
    if (!queryInfo.cache) {
      await this.invalidateHttpCache(queryEngine);
    }
    ...
    for (const key of Object.keys(newQueries)) {
      try {
        const result = await queryEngine.query(newQueries[key].query, queryContext);
        const graphQlTree = await bindingsStreamToGraphQl(await result.execute(), result.context, {materializeRdfJsTerms: true});
        results[key] = graphQlTree;
